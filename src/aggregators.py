...
class TopWords:
    ...
    def counting(self, grouped, text_col):
        ...
        """
        tqdm.pandas()
        grouped['count'] = grouped[text_col].progress_apply(lambda x: list(Counter(x).values()))
        grouped[text_col] = grouped[text_col].progress_apply(lambda x: list(Counter(x).keys()))
        return grouped
    ...
    def text_to_tokens(self, grouped, text_col, text_data, tokens_col='lemmatized', labels_col='label'):
        ...
        """
        tqdm.pandas()
        grouped['tokens'] = grouped[text_col].progress_apply(lambda x: text_data.loc[x, tokens_col].tolist())
    ...
    def find(self, main_data, text_data, col_to_group, text_col, vocab,
             ...
             mapping_df=None, max_groups=5, block_list=[]):
        """
        TODO
        """
